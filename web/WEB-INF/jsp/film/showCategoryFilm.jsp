...
<body>
	...
	<div class="content">
		<div class="col-sm-3 col-md-2 sidebar" style="height: 892px">
				...
				<div class="form-group" style="margin-top: 30px;">
					<div class="input-group input-group-md">
						...
						<span class="input-group-btn">
							<button style="background-color: #374850; border-color: #374850;outline:none;border:none"
			...
			<div class="drop-navigation drop-navigation" style="margin-top: 20px">
				<ul class="nav nav-sidebar" style="background: #272c2e">
					<li ><a href="${ ctx }/showAllFilms.do"
						class="home-icon"><span class="glyphicon glyphicon-home"
							aria-hidden="true">&nbsp;全部</span></a></li>

					<c:forEach var="item" items="${filmCategory }">
					
						<li class="${item.title == type ? 'active' : '' }"><a href="${ctx }/showCategoryFilm.action?id=${item.id }" class="user-icon"><span
		...
		<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main" style="height:892px">
							...
							<h3 class="page-header">
								...
								<c:if test="${_user.admin || _user.superAdmin }">
								 <a style="float: right; font-size: 16px; color: blue"
									href="${ctx }/upload.action"><input type="button"
									class="FilmManagement" value="影院管理" /></a>
									</c:if>
							</h3>
						</div>
						...
						<c:forEach var="item" items="${data.list }">
							<div class="col-md-3 resent-grid recommended-grid">
								<div class="resent-grid-img recommended-grid-img">
									<a
										href="${ctx }/playFilm.do?id=${item.id}"><img
								...
								<div style="box-shadow: 0px 0 2px #C1C1C1 !important;">
									<h5>
										<a style="margin-left:8px" href="${ctx }/playFilm.do?id=${item.id}" class="title">${item.title }</a>
									</h5>
									<div style="margin-left:8px">主演：${item.leadingrole }<span style="float: right;margin-right: 5px">${item.country }</span></div>
								</div> 
							</div>
						</c:forEach>
